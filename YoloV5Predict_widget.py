...
# --------------------
class YoloV5PredictWidget(core.CProtocolTaskWidget):

    def __init__(self, param, parent):
        core.CProtocolTaskWidget.__init__(self, parent)

        if param is None:
            self.parameters = processMod.YoloV5PredictParam()
        else:
        ...
        self.combo_model = utils.append_combo(self.grid_layout, "Model")
        ...
        self.browse_model = utils.BrowseFileWidget(path=self.parameters.model_path, tooltip="Select file",
        ...
        self.spin_size = utils.append_spin(self.grid_layout, "Input size", self.parameters.input_size)

        self.spin_confidence = utils.append_double_spin(self.grid_layout, "Confidence", self.parameters.conf_thres, step=0.05, decimals=2)

        self.spin_iou = utils.append_double_spin(self.grid_layout, "IOU threshold", self.parameters.iou_thres, step=0.05, decimals=2)
